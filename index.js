(function () {
	...
	const drawHTML = (el, num) => el.nextElementSibling.insertAdjacentHTML('afterend', ` - <em>(${num.toLocaleString()})</em>`);
	function main() {
		const els = getLinks();
		...
		return Promise.all(scopes.concat(getCount(query))).then(arr => Object.assign.apply(null, arr)).then(obj => {
			els.forEach(el => (el.className += ' num') && drawHTML(el, obj[el[tc]].downloads));
		});
	}
	setTimeout(main,1);
